...
public class PinyinConverter implements Converter {
	...
	@Override
	public String[] getPinyin(char ch) throws BusinessException {
		try{
	...
	@Override
	public String getPinyin(String chinese) throws BusinessException {
...
//		chinese = chinese.replaceAll("[\\.，\,\！\·\!？\\?；\;\\(\\)（）\\[\\]\\:： ]+", "").trim();
		...
		for(int i=0;i<chs.length;i++){
			...
			if(py_arr==null || py_arr.length<1){
				throw new BusinessException("pinyin array is empty, char:"+chs[i]+",chinese:"+chinese);
			}
			if(py_arr.length==1){
	...
	public String getPinyinFirstSpell(String chinese) throws BusinessException {
		String str = getPinyin(chinese);
