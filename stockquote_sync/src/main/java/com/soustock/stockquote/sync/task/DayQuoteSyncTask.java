    ...
    protected void process() throws BusinessException {
        try {
            ...
            for (StockBasicPo stockBasicPo: stockBasicPoList) {
                ...
                stockIndex ++;
                logger.info(String.format("Day quote sync: %d/%d, %s...begin", stockIndex, stockCount, stockCode));
                syncEveryStock(stockCode);
                logger.info(String.format("Day quote sync: %d/%d, %s...done.", stockIndex, stockCount, stockCode));
            }
        } catch (Exception ex) {
